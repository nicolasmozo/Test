...
@section('title')
<title>{{__('admin.All Slider')}}</title>
@endsection
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>{{__('admin.All Slider')}}</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="{{ route('admin.dashboard') }}">{{__('admin.Dashboard')}}</a></div>
              <div class="breadcrumb-item">{{__('admin.All Slider')}}</div>
            </div>
          </div>
                            ...
                            <tbody>
                                @foreach ($partners as $index => $partner)
                                    <tr>
                                        ...
                                        <td>{{ ++$index }}</td>
                                        <td><a target="_blank" href="{{ $partner->link }}">{{__('admin.Click here')}}</a></td>

                                        <td> <img class="w_200" src="{{ asset($partner->logo) }}" alt="" width="50px"></td>
                                        <td>
                                            @if($partner->status == 1)
